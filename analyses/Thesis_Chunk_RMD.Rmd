...
#note in timeline says to combine c-min and decomp data for RDA? trying to do this below
#export 2 datasets
write.csv(csi.full.ns, file="csi.full.ns.csv")
write.csv(carbon_min, file="carbonmin.csv")
#combined to include ONLY the barcodes in C-Min file (uploaded below)
csi.cmin.full <- read.csv("csi.full.ns.cmin.csv")
rownames(csi.cmin.full) <- csi.cmin.full$X
cmin.full <- subset(csi.cmin.full, select = -X)


#need these for this file?
df.bcc2 <- cmin.full[,-c(1:15)] #bacteria
df.env.bcc2 <- cmin.full[,c(1:17)] #env
df.decomp2 <- cmin.full[,c(14:16)] #decomp
df.cmin <- cmin.full[,c(1,17)]
