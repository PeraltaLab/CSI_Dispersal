...
scale_color_brewer(name="Dispersal", type = "seq", palette = "Dark2", labels = c("source only", "no dispersal", "fresh+salt", "salt only")) + labs(x = "Salinity", y = "Ammonium-N (µM)") +theme(axis.title=element_text(vjust=1,size=16,face="bold"), axis.text=element_text(size=14))
...
scale_color_brewer(name="Dispersal", type = "seq", palette = "Dark2", labels = c("source only", "no dispersal", "fresh+salt", "salt only")) + labs(x = "Salinity", y = "Nitrate+Nitrite-N (µM)") +theme(axis.title=element_text(vjust=1,size=16,face="bold"), axis.text=element_text(size=14))
...
scale_color_brewer(name="Dispersal",type = "seq", palette = "Dark2", labels = c("source only", "no dispersal", "fresh+salt (2)", "salt only (3)")) + labs(x = "Salinity", y = "Phosphate-P (µM)") +theme(axis.title=element_text(vjust=1,size=16,face="bold"), axis.text=element_text(size=14))
...
```{r (Summary - Cmin/Decomp day 45 only), echo=TRUE}
#design.env.full
str(design.env.full)
...
theme(panel.background = element_blank()) + 
  ...
  scale_colour_brewer(palette = "Dark2",type = "seq") + 
...
theme(panel.background = element_blank()) + 
  ...
  geom_errorbarh(aes(xmax=V1+V1e, xmin=V1-V1e, height=0.01), colour="black") +    
  geom_errorbar(aes(ymax=V2+V2e, ymin=V2-V2e, width=0.01), colour="black") + 
  scale_colour_brewer(labels = c("saltwater","freshwater"), type = "seq", palette = "Dark2") + 
...
```
Graph to match rest of paper
```{r}
plot1a2 <- ggplot(df1a2, aes(x=V1, y=V2)) + theme_bw() 
plot1a2 + theme(panel.grid.major = element_blank(), 
               panel.grid.minor = element_blank(), 
               axis.line = element_line(colour = "black")) + 
theme(panel.background = element_blank()) + 
  geom_point(aes(color = pcoa.col2), size=6, stroke = 0.75) + 
  geom_errorbarh(aes(xmax=V1+V1e, xmin=V1-V1e, height=0.01), colour="black") +    
  geom_errorbar(aes(ymax=V2+V2e, ymin=V2-V2e, width=0.01), colour="black") + 
  scale_colour_brewer(name = "Salinity", labels = c("saltwater","freshwater"), type = "seq", palette = "Dark2") + 
 #scale_fill_manual(labels = c("saltwater","freshwater"), 
                     #values = c("#FFFFFF", "#0066CC")) + 
  scale_shape_manual(labels = c("0","18","45"),
                     values = c(22, 21, 24)) +
