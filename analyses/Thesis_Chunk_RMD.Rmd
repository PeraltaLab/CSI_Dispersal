...
############# Remove Now> ###################
# not yet finished - data manipulation in prep for Venn
test <- csi.PA.full[c(1:25)]
test$Salinity <- as.factor(test$Salinity)
head(test)
str(test)
new.data <- reshape2::melt(test)


dcast(new.data, Replicate ~ variable, fun.aggregate = mean, measure.var = c("Otu00001"))                 
                                 ,
  variable.name = "OTU", 
  value.name = "OTU_PA")
head(new.data)

dcast(csi.PA.full, Replicate ~ variable, fun.aggregate = mean, 
  na.rm = TRUE)
...
fns <- occupants(csi_pres_abs[which(design_final$Number == 2), ])
f_only <-  occupants(csi_pres_abs[which(design_final$Number == 3), ])

draw.pairwise.venn(area1 = length(fns), area2 = length(f_only),
                    cross.area = length(intersect(fns, f_only)),
                   category = c("Fresh and Salt", "Salt"), 
...
overlap <- calculate.overlap(list(fns, sal0, sal5, sal9, sal13))

dev.off()
plot.new()
draw.quintuple.venn(c(rev(lengths(overlap))), direct.area = T, scaled = T)

dev.off()
plot.new()
draw.quintuple.venn(area1 = length(fns), area2 = length(sal0),
     area3 = length(sal5), area4 = length(sal9), area5 = length(sal13), 
     n12 = length(Reduce(intersect, list(fns, sal0))),
     n13 = length(Reduce(intersect, list(fns, sal5))), 
     n14 = length(Reduce(intersect, list(fns, sal9))),
     n15 = length(Reduce(intersect, list(fns, sal13))),
     n23 = length(Reduce(intersect, list(sal0, sal5))),
     n24 = length(Reduce(intersect, list(sal0, sal9))),
     n25 = length(Reduce(intersect, list(sal0, sal13))),
     n34 = length(Reduce(intersect, list(sal5, sal9))),
     n35 = length(Reduce(intersect, list(sal5, sal13))),
     n45 = length(Reduce(intersect, list(sal9, sal13))),
     n123 = length(Reduce(intersect, list(fns, sal0, sal5))),
     n124 = length(Reduce(intersect, list(fns, sal0, sal9))),
     n125 = length(Reduce(intersect, list(fns, sal0, sal13))), 
     n134 = length(Reduce(intersect, list(fns, sal5, sal9))),
     n135 = length(Reduce(intersect, list(fns, sal5, sal13))),
     n145 = length(Reduce(intersect, list(fns, sal9, sal13))),
     n234 = length(Reduce(intersect, list(sal0, sal5, sal9))),
     n235 = length(Reduce(intersect, list(sal0, sal5, sal13))),
     n245 = length(Reduce(intersect, list(sal0, sal9, sal13))),
     n345 = length(Reduce(intersect, list(sal5, sal9, sal13))),
     n1234 = length(Reduce(intersect, list(fns, sal0, sal5, sal9))),
     n1235 = length(Reduce(intersect, list(fns, sal0, sal5, sal13))),
     n1245 = length(Reduce(intersect, list(fns, sal0, sal9, sal13))),
     n1345 = length(Reduce(intersect, list(fns, sal5, sal9, sal13))),
     n2345 = length(Reduce(intersect, list(sal0, sal5, sal9, sal13))),
     n12345 = length(Reduce(intersect, list(fns, sal0, sal5, sal9, sal13))), 
     scaled = T)
  
dev.off()   
plot.new()
draw.quad.venn(area1 = length(sal0), area2 = length(sal5), 
                    area3 = length(sal9), area4 = length(sal13), 
     n12 = length(Reduce(intersect, list(sal0, sal5))),
     n13 = length(Reduce(intersect, list(sal0, sal9))),
     n14 = length(Reduce(intersect, list(sal0, sal13))),
     n23 = length(Reduce(intersect, list(sal5, sal9))),
     n24 = length(Reduce(intersect, list(sal5, sal13))),
     n34 = length(Reduce(intersect, list(sal9, sal13))),
     n123 = length(Reduce(intersect, list(sal0, sal5, sal9))),
     n124 = length(Reduce(intersect, list(sal0, sal5, sal13))),
     n134 = length(Reduce(intersect, list(sal0, sal9, sal13))),
     n234 = length(Reduce(intersect, list(sal5, sal9, sal13))),
     n1234 = length(Reduce(intersect, list(sal0, sal5, sal9, sal13))),
     scaled = T, category = c("sal0", "sal5", "sal9", "sal13"))
...
# PLOT GGPLOT WITH SOURCE TANKS (where is salinity 0???) - need to fix figure to show salinity 0 and SOURCES **still need to fix
```{r}
...
Venn Diagram figures
```{r}
#transpose the presence absence matrix
transposed.pres.abs <- t(csi_pres_abs.ns)
csi.full.ns

#venn diagrams
install.packages("gplots")
require(gplots)
venn(transposed.pres.abs[,7:8]) #need to figure out how to compare ALL of each salinity -- above code is salinity 13 (CSI010) compared to salinity 0 (CSI010) as an example 
venn(transposed.pres.abs[,1:2]) #this is comparing salinity 0 (CSI004) and salinity 5 (CSI005) just to see the difference in this and above venn
