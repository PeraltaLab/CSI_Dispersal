...
Chunk Description: (1) How does salinity and dispersal influence bacterial diversity?
...
#QUESTION: should I be using this data, or the rarefied data used when calculating the shannon H'?
...
(3) How does salinity and dispersal influence bacterial evenness?
Calculate Pielou's J (evenness metric)
Graph evenness
Run stat model evenness~dispersal + salinity + dispersal*salinity
Write up results in bullet points

```{r (2)}
#use formula for pielou's j -- vegan does not have its own evenness function -- Shannon H'/log(species #)

evenness <- shannon/log(specnumber(rarefyotu))
evenness

even.lm <- lm(evenness ~ Dispersal + Salinity + Dispersal*Salinity, data = design_final)

even.anova <- anova(even.lm)
summary(even.lm)
