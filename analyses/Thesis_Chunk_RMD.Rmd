...
```{r (4 w source tanks)}
#import C-mineralization CSV -- note that 0 dispersal is a source tank
...
REPEAT C_MINERALIZATION RATES WITHOUT SOURCE TANKS

```{r (4 w/o source tanks}
#import C-mineralization CSV -- note that this file has NO source tanks
...
csi.ns.relabun <- vegdist(csi_relabun, method="bray")

#PCoA
csi.pcoa <- cmdscale(csi.ns.relabun, k=3, eig=TRUE, add=FALSE)
explainvar1 <- round(csi.pcoa$eig[1] / sum(csi.pcoa$eig), 3) * 100
explainvar2 <- round(csi.pcoa$eig[2] / sum(csi.pcoa$eig), 3) * 100
sum.eig <- sum(explainvar1, explainvar2)

#NOTHING BELOW THIS POINT WORKS AS OF 9/28
treatment <- design_relabun$treatments1

points <- cbind(as.data.frame(csi.pcoa$points), treatment)
L.centroids <- melt(points, id="treatments1", measure.vars = c("0", "5","9","13"))
centroids <- cast(L.centroids, variable ~ treatments1, mean)
centroids.se <- cast(L.centroids, variable ~ treatments11, se)
...
colnames(cent.dataframe) <- c("0", "5", "9", "13")
