...
```{r (5)}
# Graphing Shannon Diversity - Treatment Salinity ***still need to fix graph legend labels and fill color 
p <- ggplot(csi_otu.div, aes(x=Salinity, y=shannon, color=as.factor(Dispersal)))+stat_summary(fun.data=mean_cl_boot,size=.5)
p1=p+geom_smooth(method="lm",aes(fill=Dispersal))+facet_wrap(~Date)
p1 + theme_bw() 
p1 + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) +theme(axis.title=element_text(vjust=1,size=16,face="bold"), axis.text=element_text(size=14), axis.text.x = element_text(vjust=0.65, hjust=0.5, size=14), panel.border = element_rect(colour = "black",size=1.25)) + theme(axis.ticks.length=unit(0.3,"cm")) + labs(x = "Salinity", y = "Shannon Diversity Index (H')")
