...
ggsave("../figures/Phylo_Ordination.png", plot=last_plot(), device=NULL, path=NULL, scale=1, width=7, height=5, dpi=900, limitsize=TRUE)
```
...
p1 + geom_flat_violin(aes(fill = group),
                      position = position_nudge(x = .1, y = 0), 
                      adjust = 1.5, trim = FALSE, alpha = .5, colour = NA) +
  geom_point(aes(x = as.numeric(time)-.15, y = PD, colour = group),
                      position = position_jitter(width = .05), size = 1.5, shape = 20) +
  geom_boxplot(aes(x = as.factor(time), y = PD, fill = group),
               outlier.shape = NA, alpha = .5, width = .1, colour = "black") + 
  scale_fill_manual(name="group", values = c("#E69F00", "#56B4E9", "#009E73","#F0E442")) + 
  scale_color_manual(name="group", values = c("#E69F00", "#56B4E9", "#009E73","#F0E442")) + 
  labs(x = "Sampling Date", y = "Phylogenetic Diversity") + 
  scale_x_discrete(labels=c("1" = "Day 0", "2" = "Day 18", "3" = "Day 45")) +
  theme_cowplot()
p1

ggsave("../figures/Figure_PD_rainclouds.png", plot=last_plot(), device=NULL, path=NULL, scale=1,  width=7, height=5, dpi=900, limitsize=TRUE)
```
...
p <- ggplot(csi_otu.div.day45, aes(x=I_PD, y=Cmin, 
                                   color = as.factor(Dispersal))) +
  scale_color_manual(name="Dispersal Treatment", 
                     values=c("black","gray"), 
                     labels = c("fresh+salt (2)", "salt only (3)")) # +
...
p <- ggplot(csi_otu.div.day45, aes(x=I_rel, y=Cmin, 
                                   color = as.factor(Dispersal))) +
  scale_color_manual(name="Dispersal Treatment", 
                     values=c("black","gray"), 
                     labels = c("fresh+salt (2)", "salt only (3)")) # +
...
p <- ggplot(csi_otu.div.day45, aes(x=I_S, y=Cmin, 
                                   color = as.factor(Dispersal))) +
  scale_color_manual(name="Dispersal Treatment", 
                     values=c("black","gray"), 
                     labels = c("fresh+salt (2)", "salt only (3)")) # +
...
ggsave("../figures/Supp_BrayCurtis_Ordination.png", plot=last_plot(), device=NULL, path=NULL, scale=1, width=7, height=5, dpi=900, limitsize=TRUE)
```
...
## Graph Cmin/Shannon day 45 only ALL OTUs - not needed for manuscript
```{r (Cmin Shannon day 45 only), eval=FALSE, include=FALSE}
csi_otu.div.day45 <- subset(csi_otu.div, Date2 == "45")
...
p <- ggplot(csi.phylo.ns, aes(x=Salinity, y=PD, color=as.factor(Dispersal))) +
     scale_color_manual(name="Dispersal Treatment", values=c("black","gray"), labels = c("fresh+salt (2)", "salt only (3)")) +
     stat_summary(fun.data=mean_cl_boot,size=1,position=position_dodge(width=2)) 
