...
require("ape"); require("picante"); require("ggtree"); 
...
min(rowSums(otu_final.ns)) #min abundance is 22349
...
#csi.unifrac.ns <- cbind(design.ns.final,unifrac.dist)- unifrac.dist is not a dataframe

# PERMANOVA UniFrac
str(unifrac.dist)
adonis <- adonis2(unifrac.dist~Salinity*Dispersal*Date, data = design_final, perm=1000, set.seed=42)
adonis

```

# Fig 2A: PCoA (UniFrac) - DONE
...
UniFrac_PCoA <- plot1a + theme(panel.grid.major = element_blank(), 
...
PD_rain <-p1 + geom_flat_violin(aes(fill = group),position = position_nudge(x = .1, y = 0),
...
# Fig 3A: PhyloTree with Indicator Taxa
...
# Add heatmap to plot
tree <- gheatmap(p, sal.dat, width = 0.5, colnames_angle=90, hjust=1, 
...
set.seed(7)
euler <- plot(euler(inds, shape = "ellipse"), quantities = TRUE, adjust_labels = T,
           fills = c("#E69F00", "#56B4E9", "#009E73","#F0E442", "gray", "gray"))
euler

# Save Euler OTUs
inds.out <- matrix(NA, nrow = length(inds), ncol = 2)
for(i in 1:length(inds)){
  temp <- paste(inds[[i]], collapse = ", ")
  temp.grp <- names(inds)[i]
  inds.out[i, ] <- c(temp.grp, temp)
}

write.table(inds.out, file = "../data/Euler-OTUs.csv", sep = ", ", 
            row.names = F, col.names = F)


#dev.copy2pdf(file = "../figures/ConcenTrait.pdf",
...
str(inds)
ind_0 <- inds[["0 psu"]]
ind_0_tax <- unique(csi.tax$Order[which(csi.tax$OTU %in% ind_0)])
ind_0_taxonomy <- csi.tax[which(csi.tax$OTU %in% ind_0), ]
table(ind_0_taxonomy$Class)

ind_5 <- inds[["5 psu"]]
ind_5_tax <- unique(csi.tax$Class[which(csi.tax$OTU %in% ind_5)])
ind_5_taxonomy <- csi.tax[which(csi.tax$OTU %in% ind_5), ]
table(ind_5_taxonomy$Class)

ind_9 <- inds[["9 psu"]]
ind_9_tax <- unique(csi.tax$Class[which(csi.tax$OTU %in% ind_9)])
ind_9_taxonomy <- csi.tax[which(csi.tax$OTU %in% ind_9), ]
table(ind_9_taxonomy$Class)

ind_13 <- inds[["13 psu"]]
ind_13_tax <- unique(csi.tax$Class[which(csi.tax$OTU %in% ind_13)])
ind_13_taxonomy <- csi.tax[which(csi.tax$OTU %in% ind_13), ]
table(ind_13_taxonomy$Class)

ind_S <- inds[[">0 psu"]]
ind_S_tax <- unique(csi.tax$Class[which(csi.tax$OTU %in% ind_S)])
ind_S_taxonomy <- csi.tax[which(csi.tax$OTU %in% ind_S), ]
table(ind_S_taxonomy$Class)
```

# ConcenTrait Analysis
...
tree.45 <- drop.tip(tree, setdiff(tree$tip.label, colnames(otus.45)),
                    trim.internal = T) #error object "phy" is not of class "phylo
...
## Fig. 3C - ConcenTrait Plot
```{r ConcentTrait, echo=TRUE}

png(filename = "../figures/ConcenTrait.png", width = 6, height = 4, units = "in", 
    res = 900, bg = "white")
...
arrows(x0 = csi.ct$Distance.L, x1 = csi.ct$Distance.L + csi.ct$CI_95.L,
       y0 = 1:5, y1 = 1:5, angle = 90, length = 0.05)
...
points(x = csi.ct$Distance.L, y = c(1:5),
     ...
     pch = 22, bg = c("#E69F00", "#56B4E9", "#009E73","#F0E442", "gray"), cex = 2)

axis(side = 1, at = c(-3, -2, -1, 0, 1),
     labels = c(0.001, 0.01, 0.1, 1, 10), cex.lab = 0.8)

axis(side = 2, at = c(1:5),
     labels = csi.ct$Trait, las = 1)

mtext("Average Cluster Depth", side = 1, line = 2.5, cex = 1.25)
mtext("Salinity Treatment", side = 2, line = 4, cex = 1.25)
...
PD_Cmin <- p + theme_bw() + theme(panel.grid.major = element_blank(), 
                        panel.grid.minor = element_blank(), 
                        axis.line = element_line(colour = "black")) + 
...
p <- ggplot(csi_otu.div.day45, aes(x=I_rel, y=Cmin, fill = as.factor(Salinity), shape = as.factor(Dispersal))) + 
  ...
  scale_fill_manual(name="Salinity (psu)", values=c("#E69F00", "#56B4E9", "#009E73","#F0E442")) +
  scale_shape_manual(name="Dispersal Treatment", values=c(21,24), labels = c("fresh+salt (2)", "salt only (3)"))
relabun_Cmin <- p + theme_bw() + theme(panel.grid.major = element_blank(), 
                        panel.grid.minor = element_blank(), 
                        axis.line = element_line(colour = "black")) + 
  theme(axis.title=element_text(vjust=1,size=16,face="bold"), 
        axis.text = element_text(size=16), 
...
# Shannon Diversity x Cmin - not in manuscript
...
# OTU richness (S) x Cmin - not in manuscript
...
PD.lmer <- lmer(PD ~ Salinity+Dispersal+Date + (1|Replicate), data = csi.phylo.ns)
plot(PD.lmer)
summary(PD.lmer, ddf="Kenward-Roger")
confint(PD.lmer)
...
OTU_PCoA <- plot1a + theme(panel.grid.major = element_blank(), 
               panel.grid.minor = element_blank(), 
               axis.line = element_line(colour = "black")) + 
theme(panel.background = element_blank()) + 
  geom_point(aes(fill=salinity), colour = "black", size=6, stroke = 1) + 
...
# PERMANOVA OTU
adonis <- adonis2(sampleREL.dist~Salinity*Dispersal*Date, data = design.ns.final, perm=1000, set.seed=42)
adonis
```
...
ggsave("../figures/Cmin_PD.png", plot=last_plot(), device=NULL, path=NULL, scale=1, width=NA, height=NA, dpi=300, limitsize=TRUE)
```
#combined Figure 2 PD graphs
```{r Loading Files, include=FALSE}
#combining plots
#UniFrac_PCoA #OTU_PCoA #PD_rain

legend <- get_legend(
  # create some space to the left of the legend
  UniFrac_PCoA + theme(legend.box.margin = margin(0, 0, 12))
)
# remove legends and axes
n1 <-UniFrac_PCoA + theme(legend.position="none")
n2 <- OTU_PCoA + theme(legend.position="none") ### first run OTU PCoA - Bray Curtis (Supp)

# align all plots horizontally
plots <- align_plots(n1,n2, align = 'h', axis = 'h')

# combine PCoA
n3 <- plot_grid(plots[[1]],plots[[2]], labels = c("A","B"), label_size = 16, nrow=1, align='h')
n3 <- plot_grid(n3, legend, rel_widths = c(8, 1.8))

n4 <- PD_rain + theme(legend.position="right")
n5 <- plot_grid(n4, labels = c("C"), label_size = 16, nrow=1, align='h')

plot_grid(n3,n5, ncol = 1)

ggsave("../figures/Figure_PCoA_PD_Combined.png", plot=last_plot(), device=NULL, path=NULL, scale=1,  width=9, height=7, dpi=500, limitsize=TRUE)
```
#combined Figure 4 graphs
```{r Loading Files, include=FALSE}
#combining plots

#PD_Cmin #relabun_Cmin 

legend <- get_legend(
  # create some space to the left of the legend
  PD_Cmin + theme(legend.box.margin = margin(0, 0, 12))
)
# remove legends and axes
n1 <-PD_Cmin + theme(legend.position="none")
n2 <- relabun_Cmin + theme(legend.position="none")
...
n4 <- plot_grid(n3, legend, rel_widths = c(8, 1.8))

n4
ggsave("../figures/Figure_PD_RelAbun_Cmin_Combined.png", plot=last_plot(), device=NULL, path=NULL, scale=1,  width=10, height=5, dpi=500, limitsize=TRUE)
```